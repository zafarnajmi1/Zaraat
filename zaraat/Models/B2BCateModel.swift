...
import Foundation
struct B2BCateModel : Codable {
    let message : String?
    let categories : [B2BCategories]?
    let status : Int?
    let success : Int?
    ...
    init(from decoder: Decoder) throws {
        ...
        categories = try values.decodeIfPresent([B2BCategories].self, forKey: .categories)
